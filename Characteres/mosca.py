...
def draw_sword(p):
    ...
    if p.hay_choque:
        set_color(*p.color_colision_actual)
    else:   
        set_color(0.4, 0.2, 0.1)
    glPushMatrix()
...
def draw_box_face(p):
    ...
    if p.hay_choque:
        set_color(1.0,1.0,1.0) # Blanco para resaltar la colisión
    else:
        set_color(0.1, 0.1, 0.1) # Color base del hocico
...
def draw_eyes(p):
    for dx in [-0.22, 0.22]:
        ...
        set_color(1, 1, 1) # Blanco para los ojos
...
def draw_mouth(p):
    if not state.is_shadow_pass:
        glDisable(GL_LIGHTING)
    glLineWidth(3)
    set_color(1, 1, 1) # Blanco para la boca
    #  lighting.set_color(1, 1, 1)
    for dx in [-0.1, 0.1]:
        glPushMatrix()
    ...
    if p.expression == "happy":
        set_color(1, 0.2, 0.2) # Rojo para feliz
        ...
        set_color(0.2, 0.2, 1) # Azul para triste
        ...
        set_color(0, 0, 0) #  Negro para gritando
        #lighting.set_color(0, 0, 0)
        glPushMatrix()
...
def draw_mosca_full(p):
    ...
    glRotatef(90, 1, 0, 0)
    set_color(0.1, 0.1, 0.1) #color del cuello
    #  lighting.set_color(0.1, 0.1, 0.1)
    ...
    set_color(0.08, 0.08, 0.08) # color de la cabeza
    #lighting.set_color(0.08, 0.08, 0.08)
    glutSolidSphere(0.55, 30, 30)
    draw_eyes(p)
    draw_mouth(p)
    # Cuernos
    set_color(0.5, 0.1, 0.8) # color de los cuernos
    ...
    for dx in [-0.3, 0.3]:
        ...
        set_color(0.1, 0.1, 0.1) # color de las piernas
        #lighting.set_color(0.1, 0.1, 0.1)
        glRotatef(90, 1, 0, 0)
        gluCylinder(quad, 0.08, 0.08, 0.7, 15, 15)
        glTranslatef(0, 0, 0.7)
        set_color(0.65, 0.45, 0.25) # color de los pies   
    ...
    for dx in [-0.7, 0.7]:
        ...
        set_color(0.1, 0.1, 0.1) # color de los brazos    
        #lighting.set_color(0.1, 0.1, 0.1)
        glRotatef(90, 1, 0, 0)
